...
20. **PII-Free repo**: NUNCA nombres reales, empresas, handles ni datos personales en codigo, docs, CHANGELOG, releases, commits ni PRs. Usar genericos (`test-org`, `alice`, `test company repo`). Detalle → `@.claude/rules/domain/pii-sanitization.md`
21. **Self-Improvement Loop**: Tras correccion del usuario o bug descubierto → escribir leccion en `tasks/lessons.md`. Revisar al inicio de sesion. Detalle → `@.claude/rules/domain/self-improvement.md`
22. **Verification Before Done**: NUNCA marcar tarea como completada sin prueba demostrable. Preguntarse "¿lo aprobaria un senior?" Detalle → `@.claude/rules/domain/verification-before-done.md`
23. **Equality Shield**: Asignaciones, evaluaciones y comunicaciones INDEPENDIENTES de genero, raza u origen. Test contrafactual obligatorio. Detalle → `@.claude/rules/domain/equality-shield.md`
