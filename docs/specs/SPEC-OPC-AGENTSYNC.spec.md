...
decisiones del agente commit-guardian) existe en `.opencode/agents/` pero NO
fue replicado a `.opencode/agents/` durante la migracion OpenCode (Era 189).

`.opencode/agents/` tiene 70 .md files. `.opencode/agents/` tiene 70 .md files
...
- **REQ-01** Directorio `.opencode/agents/decision-trees/` debe existir con
  contenido identico a `.opencode/agents/decision-trees/`.
...
- **AC-01** `diff -r .opencode/agents/decision-trees .opencode/agents/decision-trees` no muestra diferencias.
- **AC-02** `bash scripts/agents-opencode-convert.sh` completa sin error y reporta "decision-trees: OK" o "decision-trees: sync".
- **AC-03** Tras modificar un archivo en `.opencode/agents/decision-trees/`, re-ejecutar el script lo replica a `.opencode/`.
...
| `.opencode/agents/decision-trees/` | CREAR: copia desde `.opencode/agents/decision-trees/` |
...
3. **Sync tras drift**: modificar `commit-guardian-decisions.md` en `.opencode/agents/decision-trees/`. Ejecutar. Verificar replica.
