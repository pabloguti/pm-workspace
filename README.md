...
**Nidos — Terminales paralelos sin conflictos** — Ejecuta varias instancias de Claude Code en paralelo, cada una en su propia rama y worktree aislado. Los nidos se crean fuera de carpetas sincronizadas (OneDrive, Dropbox, iCloud) para evitar conflictos. Cross-platform: Windows (Git Bash) y Linux/macOS. Uso: `bash scripts/nidos.sh create feat-auth` y abre Claude Code en `~/.savia/nidos/feat-auth/`. Savia detecta en qué nido estás al iniciar sesión (`/nidos status`).
